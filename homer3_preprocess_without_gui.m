...
% Initialize once (before looping runs)
mlAct_ALL = false(92,1);  % logical is better than zeros
nRuns = numel(snirf_files);
mlAct_by_run = false(92,nRuns); % store per-run masks (92 x runs)
...
for k = 1:numel(snirf_files)
    ...
    try
        ...
        mask_r = (mlActAuto_2{1}(:,3) == 1);   % 92x1 logical

        % OR across runs (your earlier idea)
        mlAct_ALL = mlAct_ALL | mask_r;
    
        % append/store this run’s mask as a row
